...
type Field = "username" | "email" | "password" | "confirm" | "otp" | null;
type Step = "form" | "otp";
...
export default function RegisterScreen() {
  ...
  // BƯỚC 1: validate form rồi gửi mã OTP về email
  const handleSendOtp = async () => {
    ...
    try {
      ...
      toast.success("Đã gửi mã xác thực", `Kiểm tra hộp thư ${email.trim()}`);
      setStep("otp");
      startCooldown();
    } catch (e: any) {
      const message =
      ...
      setError(message);
      toast.error("Gửi mã thất bại", message);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleResend = async () => {
    ...
    try {
      await sendOtp(email.trim());
      toast.success("Đã gửi lại mã", `Kiểm tra hộp thư ${email.trim()}`);
      startCooldown();
    } catch (e: any) {
      const message =
        e?.response?.data?.error || "Không thể gửi lại mã, vui lòng thử lại";
      setError(message);
    }
  };
              ...
              {step === "form" ? (
                ...
                <>
                  <TouchableOpacity
                    ...
                    hitSlop={8}
                  >
                    <Ionicons name="arrow-back" size={18} color="#9a9a9a" />
                    <Text style={styles.backBtnText}>{"Quay lại"}</Text>
                  </TouchableOpacity>

                  <Text style={styles.title}>{"Xác thực email"}</Text>
                  <Text style={styles.subtitle}>
                    {`Nhập mã 6 số đã gửi đến ${email.trim()}`}
                  </Text>
                  ...
                  <View style={styles.footerRow}>
                    ...
                    <TouchableOpacity onPress={handleResend} hitSlop={8}>
                      <Text
                        style={[
                          styles.footerLink,
                          resendCooldown > 0 && styles.footerLinkDisabled,
                        ]}
                      >
                        {resendCooldown > 0
...
const styles = StyleSheet.create({
  ...
  backBtnText: { color: "#9a9a9a", fontSize: 13 },
  footerRow: {
    flexDirection: "row",
    justifyContent: "center",
    marginTop: 22,
